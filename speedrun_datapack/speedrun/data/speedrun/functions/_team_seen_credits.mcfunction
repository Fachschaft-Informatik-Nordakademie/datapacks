...
$execute if score $(current) teamSeenCredits = $(current) teamMembers run execute as @a[team=$(current), tag=!seenWinningScreen] run function speedrun:_winning_screen {"winner":"$(current)"}
