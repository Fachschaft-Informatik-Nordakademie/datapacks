...
function speedrun:set_current_team with storage minecraft:teams

# ececute given command with team name
$function speedrun:execute_with_current_teamname {"function":"$(function)"}

# go to next team
scoreboard players add loop_current team_loop 1

# go to next team
$execute unless score loop_break team_loop matches 1.. unless score loop_current team_loop > loop_finish team_loop run function speedrun:team_loop {"function":"$(function)"}
